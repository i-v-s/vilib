...
class TorchFrame {
public:
  TorchFrame(torch::Tensor img,
        const int64_t timestamp_nsec,
        const std::size_t n_pyr_levels/*,
        cudaStream_t stream = 0*/);
#ifdef ROS_SUPPORT
  TorchFrame(const sensor_msgs::ImageConstPtr & msg,
        const std::size_t n_pyr_levels,
        cudaStream_t stream = 0);
#endif /* ROS_SUPPORT */
  ~TorchFrame(void);
  ...
  std::vector<std::shared_ptr<Subframe>> pyramid_gpu();
...
private:
  TorchFrame(const int64_t timestamp_nsec,
